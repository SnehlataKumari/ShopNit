...
function HotDealCards() {
  return (
    <>
      <div width="1024px">
        {
          [[men, Women], [Hoodie, Comfy], [PlainTShirt, PoloTShirt], [Laptop, CoffeeMug]].map((row, i) => (
            <Row key={i} gutter={30} style={{ marginLeft: "15%", paddingTop: i === 0 ? "0px" : '20px' }} >
              {
                row.map((col, i) => (
                  <Col key={i} className="gutter-row" span={7} style={{ paddingLeft: (((i+1) % 2) === 0) ? '170px' : '20px' }} >
